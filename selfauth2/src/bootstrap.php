...
// ---------------------------------------------------------------------
// External (OIDC/OAuth2) authentication. Two independent toggles:
// SELFAUTH_LOGIN_OIDC_ENABLED gates SSO on the IndieAuth "me" endpoint
// itself (index.php) -- other websites will see you authenticate via
// Kanidm. SELFAUTH_ADMIN_OIDC_ENABLED gates SSO on the /admin/ portal,
// which also accepts delegate identities (see src/Delegates.php).
// Both stay hard-disabled until an owner allow-list (or, for admin,
// a delegate) exists -- see OidcClient::isEnabled().
// ---------------------------------------------------------------------
define('SELFAUTH_LOGIN_OIDC_ENABLED', OidcClient::isEnabled('login'));
define('SELFAUTH_LOGIN_OIDC_MISCONFIGURED', OidcClient::isMisconfigured('login'));
define('SELFAUTH_OIDC_ADMIN_ENABLED', OidcClient::isEnabled('admin'));
define('SELFAUTH_OIDC_MISCONFIGURED', OidcClient::isMisconfigured('admin'));

// SELFAUTH_ADMIN_AUTH_MODE: "password" (default), "oidc", or "both".
// "oidc" hides the password form -- but only once OIDC is confirmed
// enabled and configured, so a typo in the env vars can never lock you
// out of your own admin portal.
$authMode = strtolower(getenv('SELFAUTH_ADMIN_AUTH_MODE') ?: 'both');
if (!in_array($authMode, ['password', 'oidc', 'both'], true)) {
    $authMode = 'both';
}
define('SELFAUTH_ADMIN_AUTH_MODE', $authMode);
define('SELFAUTH_ADMIN_SHOW_PASSWORD_FORM', !(SELFAUTH_ADMIN_AUTH_MODE === 'oidc' && SELFAUTH_OIDC_ADMIN_ENABLED));
define('SELFAUTH_ADMIN_SHOW_OIDC_BUTTON', SELFAUTH_OIDC_ADMIN_ENABLED);

// Same idea for the IndieAuth login screen itself: SSO is additive by
// default (password form stays visible) unless explicitly narrowed.
$loginMode = strtolower(getenv('SELFAUTH_LOGIN_AUTH_MODE') ?: 'both');
if (!in_array($loginMode, ['password', 'oidc', 'both'], true)) {
    $loginMode = 'both';
}
define('SELFAUTH_LOGIN_AUTH_MODE', $loginMode);
define('SELFAUTH_LOGIN_SHOW_PASSWORD_FORM', !(SELFAUTH_LOGIN_AUTH_MODE === 'oidc' && SELFAUTH_LOGIN_OIDC_ENABLED));
define('SELFAUTH_LOGIN_SHOW_OIDC_BUTTON', SELFAUTH_LOGIN_OIDC_ENABLED);

define('SELFAUTH_CUSTOM_CSS', $settings->get('custom_css', '') ?? '');
