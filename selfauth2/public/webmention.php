...
if ($method === 'POST') {
    ...
    if (!$rateLimiter->allow('webmention_post:' . $clientIp, 30, 60)) {
        Support::errorPage('Too Many Requests', 'Please slow down.', '429 Too Many Requests');
    }
    ...
    EventBus::fire($pdo, 'webmention.received', ['source' => $source, 'target' => $target], 'New webmention received for ' . $target);
    ...
    if ($webmention->verify($result['id'], 5)) {
        EventBus::fire($pdo, 'webmention.verified', ['source' => $source, 'target' => $target], 'Webmention verified for ' . $target);
    }
